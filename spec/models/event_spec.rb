...
RSpec.describe Event, type: :model do
  ...
  it "is invalid without a start date" do 
    expect(FactoryBot.build(:event, start_date: nil, end_date: Faker::Time.forward(360, :morning))).to be_invalid
  end
  it "is invalid without a end date" do 
    expect(FactoryBot.build(:event, end_date: nil)).to be_invalid
  end
  it "is invalid if end date is the same as start date or before start date" do
    expect(FactoryBot.build(:event, start_date: Time.now, end_date: Time.now)).to be_invalid
    expect(FactoryBot.build(:event, start_date: Time.now, end_date: Time.now - 3.days )).to be_invalid
  end
  
  # Test for event status switch based on date
  it "is pending if it happens in a future year" do
    FactoryBot.create(:event, start_date: Time.now + 2.years, end_date: Time.now + 2.years + 3.days)
    @event = Event.first    
  ...
  it "is active if it happens this year and far enough in the future" do
    ...
    FactoryBot.create(:event, start_date: Time.now + 2.days, end_date: Time.now + 5.days)
    @event = Event.first
...
#  end
  ...
  it "is past if the end_date is earlier than Time.now" do
    FactoryBot.create(:event, start_date: Time.now - 5.days, end_date: Time.now - 3.days)
    @event = Event.first
...
#  end
  ...
  it "is running if today is between start_date and end_date" do
    FactoryBot.create(:event, start_date: Time.now - 1.day, end_date: Time.now + 2.days)
    @event = Event.first
